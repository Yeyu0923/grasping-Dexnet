...
class Image:
    ...
    def make_boundingbox(self, boundingbox=None, depth=False):
        if boundingbox :
            x = self.img.shape
            ...
            for i in range(x[0]):
                for j in range(x[1]):
                    # print(type(boundingbox.bounding_boxes))
                        l = len(boundingbox.bounding_boxes)
                        for k in range(l):
                            if i < int(boundingbox.bounding_boxes[k].ymax) and i>int(boundingbox.bounding_boxes[k].ymin) and j<int(boundingbox.bounding_boxes[k].xmax) and j>int(boundingbox.bounding_boxes[k].xmin):
                                t = 1
                        if t == 0 and depth==False:
                            self.img[i,j] = 0
                        if t == 0 and depth==True:
                            self.img[i,j] = 0
                        t = 0
