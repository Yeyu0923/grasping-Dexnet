...
def save_results(rgb_img, grasp_q_img, grasp_angle_img, depth_img=None, no_grasps=1, grasp_width_img=None,save_path = '/home/xfy/Dex-Net/grasping-master/src/gqcnn-master/roboticgrasping/results/'):
    """
    Plot the output of a network
    :param rgb_img: RGB Image
    :param depth_img: Depth Image
    ...
    # pub = rospy.Publisher("/GR_convNet/grasp_img",GR_img,queue_size=10)
    thread = threading.Thread(target=plat(rgb_img, grasp_q_img, grasp_angle_img, gs, save_path,depth_img, no_grasps, grasp_width_img))
    thread.start()
    return gs

def plat(rgb_img, grasp_q_img, grasp_angle_img, gs, save_path ,depth_img=None, no_grasps=1, grasp_width_img=None):
    fig = plt.figure(figsize=(10, 10))

    plt.ioff()
    plt.clf()
    ax = plt.subplot(111)
    # rgb_img.transpose(0,2)
    # rgb_img.permute(1,2,0)
    # rgb_img=np.transpose(rgb_img,(2,0))
    ...
    fig.savefig(save_path+'quality.png')
    ...
    ax.set_title('Width')
    ax.axis('off')
    fig.savefig(save_path+'width.png')
